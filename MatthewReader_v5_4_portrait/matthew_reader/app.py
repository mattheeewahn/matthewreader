...
STYLE="""
*{font-family:'DejaVu Sans';font-size:13px;color:#202020}
QMainWindow,QWidget{background:#f5f3ed}
QPushButton,QToolButton{
    background:#f5f3ed;border:1px solid #bcb8ae;border-radius:7px;
    padding:6px 8px;min-height:34px
}
QPushButton:pressed,QToolButton:pressed{background:#dedad0}
...
QLabel#docTitle{font-size:16px;font-weight:600}
QLabel#section{font-size:17px;font-weight:600;padding-top:8px}
QLabel#muted{color:#777268;font-size:11px}
QLineEdit,QComboBox,QListWidget{
    background:#fffefa;border:1px solid #bbb7ae;border-radius:6px;padding:7px;
    min-height:32px
}
QFrame#card{background:#fffefa;border:1px solid #d0ccc2;border-radius:10px}
...
class SettingsPage(QWidget):
    ...
    def refresh(self):
        ...
        self.device_label.setText(
            f"Matthew Reader v5.4 Portrait\n"
...
class LibraryPage(QWidget):
    """Portrait-first 720x1280 library layout."""
    def __init__(self,app):
        super().__init__()
        self.app=app
        ...
        # Row 1: brand + actions. Search gets its own full-width row below.
        head=QHBoxLayout()
        title=QLabel("Matthew")
        title.setObjectName("title")
        ...
        settings.clicked.connect(app.go_settings)
        ...
        self.search=TouchLineEdit("Search Library")
        self.search.setPlaceholderText("Search library")
        self.search.committed.connect(lambda _:self.refresh())
        v.addWidget(self.search)

        # Filters use the full portrait width.
        filters=QHBoxLayout()
        self.category=QComboBox()
        for label,key in [
            ("All","all"),("Recent","recent"),("Favorites","favorites"),
            ("Textbooks","textbooks"),("Notes","notes"),("Scratch","scratch")
        ]:
        ...
        self.subject.currentIndexChanged.connect(self.refresh)
        filters.addWidget(self.subject,1)

        org=QPushButton("Organize")
        org.clicked.connect(self.organize)
        filters.addWidget(org)
        v.addLayout(filters)

        self.usb=QLabel("")
        self.usb.setObjectName("muted")
        v.addWidget(self.usb)
    ...
    def poll_usb(self):
        m=usb_mounts()
        self.usb.setText(("USB: "+", ".join(x.name for x in m)) if m else "")

    def refresh_subjects(self):
        cur=self.subject.currentData() if self.subject.count() else ""
        self.subject.blockSignals(True)
        self.subject.clear()
        self.subject.addItem("All subjects","")
        for name in subjects():
            self.subject.addItem(name,name)
        idx=self.subject.findData(cur)
        self.subject.setCurrentIndex(max(0,idx))
    ...
    def refresh(self,*_):
        ...
        if not items:
            empty=QLabel("No documents in this view.")
            empty.setAlignment(Qt.AlignmentFlag.AlignCenter)
            grid.addWidget(empty,0,0,1,2)
        else:
            # 2 portrait-friendly columns at 720 px wide.
            for i,it in enumerate(items):
                ...
                if it["type"]=="pdf":
                    ...
                    if cp:
                        card.setIcon(QIcon(str(cp)))
                        card.setIconSize(QSize(105,140))
                ...
                foot=QHBoxLayout()
                sub=QLabel(it.get("subject") or ("Textbook" if it["type"]=="pdf" else it["type"].title()))
                sub.setObjectName("muted")
                foot.addWidget(sub,1)
                star=QPushButton("★" if it.get("favorite") else "☆")
                star.setFixedSize(42,38)
                star.clicked.connect(lambda _,p=it["path"]:self.favorite(p))
                foot.addWidget(star)
                lv.addLayout(foot)
        ...
        grid.setRowStretch((len(items)+1)//2,1)
    ...
    def organize(self):
        items=list_library()
        if not items:
            QMessageBox.information(self,"Organize","No documents yet.")
            return

        labels=[f"{x['title']}  [{x['type']}]" for x in items]
        choice,ok=QInputDialog.getItem(self,"Organize","Document:",labels,0,False)
        if not ok:
            return

        item=items[labels.index(choice)]
        existing=subjects()
        choices=["No subject"]+existing+["+ New subject…"]
        ...
        set_subject(item["path"],sub)
        self.refresh_subjects()
        self.refresh()

    def new_menu(self):
        m=QMenu(self)
        m.addAction("Notebook",lambda:self.create_note(".matthew","matthew"))
        m.addAction("Scratch Note",lambda:self.create_note(".matthewscr","scratch"))
        m.addSeparator()
        m.addAction("Import from USB",self.import_usb)
        m.addAction("Import PDF",self.import_pdf)
        m.exec(QCursor.pos())
    ...
    def create_note(self,ext,kind):
        ...
        tmpl,ok=QInputDialog.getItem(self,"Paper","Template:",templates,0,False)
        if not ok:
            return
        ...
        save_note(p,name.strip(),[[]],kind,tmpl)
        self.refresh()
    ...
    def import_usb(self):
        ...
        if not files:
            QMessageBox.information(self,"USB","No supported files found.")
            return

        names=[str(p) for p in files]
        choice,ok=QInputDialog.getItem(self,"USB Import","File:",names,0,False)
        if ok and choice:
            copy_into_library(Path(choice),BOOKS_DIR,NOTES_DIR)
            self.refresh()

    def import_pdf(self):
        start=str(usb_mounts()[0]) if usb_mounts() else str(Path.home())
        f,_=QFileDialog.getOpenFileName(self,"Import PDF",start,"PDF (*.pdf)")
        if f:
            copy_into_library(Path(f),BOOKS_DIR,NOTES_DIR)
            self.refresh()
...
class NotePage(QWidget):
    """Portrait-native note editor. All v5 tools retained."""
    def __init__(self,app,path):
        ...
        self.page=0
        self.template=self.manifest.get("template","blank")
        ...
        header.addWidget(self.page_lbl)

        exp=QPushButton("Export")
        exp.clicked.connect(self.export_pdf)
        header.addWidget(exp)
        v.addLayout(header)

        self.canvas=InkCanvas()

        # Main tools: split into rows so nothing depends on landscape width.
        row1=QHBoxLayout()
        for label,tool in [
            ("Pen","pen"),("Highlighter","highlighter"),
            ("Eraser","eraser"),("Lasso","lasso")
        ]:
            b=QPushButton(label)
            b.clicked.connect(lambda _,t=tool:self.canvas.set_tool(t))
            row1.addWidget(b,1)
        v.addLayout(row1)

        row2=QHBoxLayout()
        for label,tool in [("Text","text"),("Auto Shape","autoshape")]:
            b=QPushButton(label)
            b.clicked.connect(lambda _,t=tool:self.canvas.set_tool(t))
            row2.addWidget(b,1)
        ...
        for label,tool in [
            ...
            b=QPushButton(label)
            b.clicked.connect(lambda _,t=tool:self.canvas.set_tool(t))
            shapes.addWidget(b,1)
        v.addLayout(shapes)

        # Presets and pen width.
        presets=QHBoxLayout()
        for label,tool,c,w in [
            ("P1","pen","#202020",2.0),
            ("P2","pen","#2855d9",2.6),
            ("P3","pen","#d64242",3.2),
            ("H","highlighter","#ffe26a",3.0)
        ]:
            b=QPushButton(label)
            b.clicked.connect(
                lambda _,t=tool,col=c,wd=w:self.canvas.set_preset(t,col,wd)
            )
            presets.addWidget(b)

        width=QComboBox()
        width.addItems(["1.5","2.6","4.0","6.0"])
        width.setCurrentText("2.6")
        width.currentTextChanged.connect(lambda x:self.canvas.set_width(float(x)))
        presets.addWidget(width,1)
        v.addLayout(presets)

        # Manual colors remain available.
        colors=QHBoxLayout()
        for label,c in [
            ("Black","#202020"),("Blue","#2855d9"),("Red","#d64242")
        ]:
            b=QPushButton(label)
            b.clicked.connect(lambda _,x=c:self.canvas.set_color(x))
            colors.addWidget(b,1)
        v.addLayout(colors)

        self.canvas.set_page(self.pages[0],self.template)
        self.canvas.changed.connect(self.save)
    ...
    def save(self):
        self.pages[self.page]=self.canvas.objects
        save_note(
            self.path,
    ...
    def showp(self):
        self.canvas.set_page(self.pages[self.page],self.template)
        self.label()
    ...
    def export_pdf(self):
        self.save()
        try:
            created=export_note(self.path)
            dest=choose_export(self,created)
            if dest:
...
class PDFPage(QWidget):
    """Portrait-native PDF reader/editor. All v5 PDF functions retained."""
    def __init__(self,app,path):
        super().__init__()
        self.app=app
        self.path=Path(path)
        ...
        # Reader actions split into portrait-safe rows.
        row1=QHBoxLayout()
        actions=[
            ("Read",lambda:self.viewer.set_mode("read")),
            ("Pen",lambda:self.viewer.set_pen()),
            ("Undo",self.viewer.undo),
            ("TOC",self.show_toc),
        ...
        row2.addWidget(bookmark,1)

        bookmarks=QPushButton("Bookmarks")
        bookmarks.clicked.connect(self.show_bookmarks)
        row2.addWidget(bookmarks,1)

        reset=QPushButton("100%")
        reset.clicked.connect(self.viewer.reset_zoom)
        row2.addWidget(reset,1)

        go=QPushButton("Go")
        go.clicked.connect(self.goto_page)
        row2.addWidget(go,1)

        exp=QPushButton("Export")
        exp.clicked.connect(self.export_pdf)
        row2.addWidget(exp,1)
        v.addLayout(row2)

        # Pen presets and highlights.
        row3=QHBoxLayout()
        for label,c,w in [
            ("P1","#202020",2.0),("P2","#2855d9",2.6),("P3","#d64242",3.2)
        ]:
            b=QPushButton(label)
            b.clicked.connect(lambda _,col=c,wd=w:self.viewer.set_pen(col,wd))
            row3.addWidget(b,1)
        ...
        self.viewer.pageChanged.connect(
            lambda a,b:self.pg.setText(f"{a}/{b}")
        )
        self.viewer.zoomChanged.connect(
            lambda z:self.zoom.setText(f"{round(z*100)}%")
        )
        v.addWidget(self.viewer,1)

        self.pg.setText(f"{self.viewer.page+1}/{len(self.viewer.doc)}")
        self.zoom.setText(f"{round(self.viewer.zoom*100)}%")
    ...
    def goto_page(self):
        ...
        try:
            ...
            return
        if 1<=n<=len(self.viewer.doc):
            self.viewer.goto_page(n-1)
        else:
            QMessageBox.warning(
                self,"Page",f"Enter a page from 1 to {len(self.viewer.doc)}."
            )
    ...
    def show_toc(self):
        ...
        if not toc:
            QMessageBox.information(
                self,"Contents","This PDF has no embedded table of contents."
            )
            return
        labels=[("  "*(lvl-1))+title for lvl,title,page,*_ in toc]
        choice,ok=QInputDialog.getItem(
            self,"Contents","Chapter:",labels,0,False
        )
        if ok:
            self.viewer.goto_page(max(0,toc[labels.index(choice)][2]-1))
    ...
    def search_pdf(self):
        ...
        if not hits:
            ...
            return
        labels=[f"p. {p+1} — {txt[:100]}" for p,txt in hits]
        choice,ok=QInputDialog.getItem(
            self,"Search results","Result:",labels,0,False
        )
        if ok:
            self.viewer.goto_page(hits[labels.index(choice)][0])

    def bookmark(self):
        self.viewer.toggle_bookmark()
        QMessageBox.information(
            self,
            "Bookmark",
            "Bookmark "+("added." if self.viewer.is_bookmarked() else "removed.")
        )
    ...
    def show_bookmarks(self):
        ...
        if not pages:
            ...
            return
        labels=[f"Page {p+1}" for p in pages]
        choice,ok=QInputDialog.getItem(
            self,"Bookmarks","Go to:",labels,0,False
        )
        if ok:
            self.viewer.goto_page(pages[labels.index(choice)])

    def export_pdf(self):
        try:
            self.viewer.persist()
            created=export_annotated_pdf(self.path)
            dest=choose_export(self,created)
            if dest:
                QMessageBox.information(self,"Export",f"Exported to:\n{dest}")
        except Exception as e:
