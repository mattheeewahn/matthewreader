...
class ReaderWindow(QMainWindow):
    ...
    def eventFilter(self,obj,event):
        if event.type() in (QEvent.Type.TouchBegin,QEvent.Type.MouseButtonPress,QEvent.Type.KeyPress):
            if self.in_sleep:
                self.wake()
                return True
            self.reset_idle_timer()
...
class LibraryPage(QWidget):
    def __init__(self,app):
        ...
        settings.setFixedWidth(52)
        settings.clicked.connect(app.go_settings)
        top.addWidget(settings)

        b=QPushButton("+ New")
        b.setObjectName("primary")
        b.clicked.connect(self.new_menu)
        top.addWidget(b)
        v.addLayout(top)
    ...
    def refresh(self,*_):
        ...
        g=QGridLayout(host)
        g.setSpacing(12)
        items=list_library(self.search.text() if hasattr(self,"search") else "")
        if not items:g.addWidget(QLabel("No documents yet."),0,0)
        for i,it in enumerate(items):
            card=QPushButton(f"{it['title']}\n\n{'BOOK' if it['type']=='pdf' else it['type'].upper()}")
...
class NotePage(QWidget):
    def __init__(self,app,path):
        ...
        nav.addWidget(back)
        nav.addWidget(QLabel(self.manifest.get("title",self.path.stem)),1)
        ...
        tools=QHBoxLayout()
        for label,tool in [("Pen","pen"),("Highlighter","highlighter"),("Eraser","eraser"),
                           ("Lasso","lasso"),("Text","text"),("Line","line"),("Rect","rectangle")]:
            b=QPushButton(label)
        ...
        opts.addWidget(width)
        u=QPushButton("Undo");u.clicked.connect(self.canvas.undo);opts.addWidget(u)
        r=QPushButton("Redo");r.clicked.connect(self.canvas.redo);opts.addWidget(r)
        d=QPushButton("Delete");d.clicked.connect(self.canvas.delete_selection);opts.addWidget(d)
        v.addLayout(opts)
        ...
        foot=QHBoxLayout()
        pr=QPushButton("‹");pr.clicked.connect(self.prev);foot.addWidget(pr)
        nx=QPushButton("›");nx.clicked.connect(self.next);foot.addWidget(nx)
        add=QPushButton("+ Page");add.clicked.connect(self.add_page);foot.addWidget(add)
        foot.addStretch()
        v.addLayout(foot)
        self.label()
    ...
    def save(self):
        ...
        save_note(self.path,self.manifest.get("title",self.path.stem),self.pages,
                  self.manifest.get("kind","matthew"),self.template)
...
class PDFPage(QWidget):
    def __init__(self,app,path):
        ...
        back.clicked.connect(app.go_library)
        nav.addWidget(back)
        nav.addWidget(QLabel(self.path.stem),1)
        self.pg=QLabel()
        nav.addWidget(self.pg)
        self.zoom=QLabel()
        ...
        tools=QHBoxLayout()
        for label,fn in [("Read",lambda:self.viewer.set_mode("read")),("Pen",lambda:self.viewer.set_pen()),
                         ("Undo",self.viewer.undo),("TOC",self.show_toc),("Search",self.search_pdf),
                         ("Bookmark",self.bookmark)]:
            b=QPushButton(label);b.clicked.connect(fn);tools.addWidget(b)
        reset=QPushButton("100%");reset.clicked.connect(self.viewer.reset_zoom);tools.addWidget(reset)
        v.addLayout(tools)

        hi=QHBoxLayout()
        hi.addWidget(QLabel("Highlight"))
        for label,c in [("Y","yellow"),("G","green"),("B","blue"),("P","pink")]:
            b=QPushButton(label);b.clicked.connect(lambda _,x=c:self.viewer.set_highlight(x));hi.addWidget(b)
        bm=QPushButton("Bookmarks");bm.clicked.connect(self.show_bookmarks);hi.addWidget(bm);hi.addStretch()
        v.addLayout(hi)

        self.viewer.pageChanged.connect(lambda a,b:self.pg.setText(f"{a}/{b}"))
        self.viewer.zoomChanged.connect(lambda z:self.zoom.setText(f"{round(z*100)}%"))
    ...
    def show_toc(self):
        toc=self.viewer.toc()
        if not toc:
            QMessageBox.information(self,"Contents","This PDF has no embedded table of contents.")
            return
        labels=[("  "*(lvl-1))+title for lvl,title,page,*_ in toc]
        choice,ok=QInputDialog.getItem(self,"Contents","Chapter:",labels,0,False)
    ...
    def search_pdf(self):
        ...
        if not hits:
            ...
            return
        labels=[f"p. {p+1} — {s[:100]}" for p,s in hits]
        choice,ok=QInputDialog.getItem(self,"Search results","Result:",labels,0,False)
        if ok:self.viewer.goto_page(hits[labels.index(choice)][0])

    def bookmark(self):
        self.viewer.toggle_bookmark()
        QMessageBox.information(self,"Bookmark","Bookmark "+("added." if self.viewer.is_bookmarked() else "removed."))
    ...
    def show_bookmarks(self):
        ...
        choice,ok=QInputDialog.getItem(self,"Bookmarks","Go to:",labels,0,False)
