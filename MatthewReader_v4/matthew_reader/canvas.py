...
class InkCanvas(QWidget):
    changed=Signal()
    textRequested=Signal(float,float)

    def __init__(self,parent=None):
        super().__init__(parent)
        self.setAttribute(Qt.WidgetAttribute.WA_AcceptTouchEvents,True)
        self.objects=[]; self.tool="pen"; self.color="#202020"; self.pen_width=2.6
        self.template="blank"; self.current=None; self.start=None
        self.lasso=None; self.selected=[]; self.moving=False; self.move_last=None
        self.undo_stack=[]; self.redo_stack=[]

    def set_page(self,objects,template="blank"):
        self.objects=objects; self.template=template; self.selected=[]
        self.undo_stack=[]; self.redo_stack=[]; self.update()
    ...
    def undo(self):
        ...
        self.objects[:]=self.undo_stack.pop();self.selected=[];self.changed.emit();self.update()

    def redo(self):
        if not self.redo_stack:return
        self.undo_stack.append(copy.deepcopy(self.objects))
        self.objects[:]=self.redo_stack.pop();self.selected=[];self.changed.emit();self.update()
    ...
    def bbox(self,o):
        ...
        if typ=="text":
            return QRectF(o["x"]*self.width(),o["y"]*self.height()-24,max(60,len(o.get("text",""))*10),34)
        if typ=="shape":
            return QRectF(self.denorm(o["a"]),self.denorm(o["b"])).normalized().adjusted(-8,-8,8,8)
        return QRectF()

    def add_text(self,x,y,text):
        if not text:return
        self.snapshot();self.objects.append({"type":"text","x":x,"y":y,"text":text,"color":self.color,"size":18})
        self.changed.emit();self.update()

    def delete_selection(self):
        if not self.selected:return
        self.snapshot();sel=set(self.selected);self.objects[:]=[o for i,o in enumerate(self.objects) if i not in sel]
        self.selected=[];self.changed.emit();self.update()
    ...
    def press(self,pos):
        ...
        if self.tool=="lasso":
            if self.selected:
                ...
                if r.adjusted(-12,-12,12,12).contains(pos):
                    self.snapshot();self.moving=True;self.move_last=QPointF(pos);return
            self.start=QPointF(pos);self.lasso=QRectF(pos,pos);self.selected=[];self.update();return
        if self.tool in ("line","rectangle"):
            self.snapshot();self.current={"type":"shape","shape":self.tool,"a":self.norm(pos),"b":self.norm(pos),"color":self.color,"width":self.pen_width}
            self.objects.append(self.current);return
        self.snapshot()
        self.current={"type":"stroke","tool":"highlighter" if self.tool=="highlighter" else "pen",
                      "color":self.color,"width":self.pen_width*(5 if self.tool=="highlighter" else 1),
                      "points":[self.norm(pos)]}
        self.objects.append(self.current);self.update()

    def move(self,pos):
        if self.tool=="eraser":self.erase(pos);return
        if self.tool=="lasso":
            if self.moving:
                d=pos-self.move_last;self.move_last=QPointF(pos);dx=d.x()/self.width();dy=d.y()/self.height()
                for i in self.selected:
                    o=self.objects[i];typ=o.get("type")
                    if typ=="stroke":o["points"]=[[x+dx,y+dy] for x,y in o["points"]]
                    elif typ=="text":o["x"]+=dx;o["y"]+=dy
                    elif typ=="shape":
                        o["a"]=[o["a"][0]+dx,o["a"][1]+dy];o["b"]=[o["b"][0]+dx,o["b"][1]+dy]
                self.update();return
    ...
    def release(self,pos):
        if self.tool=="lasso":
            if self.moving:
                self.moving=False;self.move_last=None;self.changed.emit();self.update();return
            if self.lasso is not None:
                self.selected=[i for i,o in enumerate(self.objects) if self.lasso.intersects(self.bbox(o))]
                self.lasso=None;self.start=None;self.update()
            return
        if self.current:
            if self.current["type"]=="shape":self.current["b"]=self.norm(pos)
            else:self.current["points"].append(self.norm(pos))
    ...
    def event(self,e):
        ...
        if t in (QEvent.Type.TouchBegin,QEvent.Type.TouchUpdate,QEvent.Type.TouchEnd,QEvent.Type.TouchCancel):
            ...
            if len(pts)>=2:
                self.current=None;e.accept();return True
    ...
    def paint_template(self,p):
        ...
        if self.template=="lined":
            ...
            p.drawLine(int(self.width()*.28),30,int(self.width()*.28),self.height()-80)
            p.drawLine(20,self.height()-80,self.width()-20,self.height()-80)
            p.drawLine(20,60,self.width()-20,60)
    ...
    def paintEvent(self,e):
        ...
        for o in self.objects:
            ...
            if typ=="stroke":
                ...
                p.setPen(QColor(o.get("color","#202020")));f=QFont();f.setPointSize(int(o.get("size",18)));p.setFont(f)
                p.drawText(QPointF(o["x"]*self.width(),o["y"]*self.height()),o.get("text",""))
            elif typ=="shape":
                p.setPen(QPen(QColor(o.get("color","#202020")),float(o.get("width",2.6))))
                a=self.denorm(o["a"]);b=self.denorm(o["b"])
                p.drawLine(a,b) if o.get("shape")=="line" else p.drawRect(QRectF(a,b).normalized())
        if self.lasso:
            p.setPen(QPen(QColor("#666"),1,Qt.PenStyle.DashLine));p.drawRect(self.lasso)
        if self.selected:
